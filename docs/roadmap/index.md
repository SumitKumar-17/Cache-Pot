...
Cache-Pot's original build arc shipped as seven releases, all now out. Each
release is additive — later ones build on storage/vector/memory primitives
introduced earlier — and each is honestly scoped rather than compressed to
look smaller than it is. v0.6.0 and v0.7.0 in particular were large,
...
**Not supported** (see [Redis Compatibility](/architecture/redis-compatibility)
...
- A native MCP server (streamable HTTP, `--mcp-port`) exposing
  ...
  vision is intentionally **not** exposed: it needs the consolidation
  machinery v0.6.0 introduced, and faking it earlier would have violated
  the project's own honesty policy. `remember`/`recall` landed in v0.4.0
  below.
...
- Real memory domain layer: short-term, long-term, episodic, and semantic
  memory kinds, indexed via the v0.3.0 vector store for semantic search
...
- Version is bumped on every `MEMORY.PUT` to the same id; full version
  history retrieval was added later in v0.7.0's `MEMORY.HISTORY`
...
This was the biggest, most research-adjacent piece of work in the whole
build — entity/relationship extraction quality and consolidation judgment
calls are genuinely not a weekend feature. Both halves are real, both built
on a capability this release introduced: `internal/llm.CompletionProvider`
— [LLM completions](/getting-started/completions), Cache-Pot's first
text-*generation* provider (everything before this only ever produced
embeddings).

- **Consolidation (`SUMMARY.CREATE`):** dedup of near-duplicate memories
  ...
  **Not automatic/nightly** — this is an on-demand command; running it on a
  ...
  today.
- **Knowledge Graph (`GRAPH.EXTRACT`/`GRAPH.RELATED`):** real
...
`workspace` dimension across every subsystem built up to this point, which
is why it landed last rather than assumed free if bolted on early.
...
- **Workspace isolation:** real, enforced via `--workspace-credentials`
  ...
  with `NOPERM`. Mutually exclusive with v0.1.0's single shared
